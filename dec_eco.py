...
if is_wandb:
    wandb.init(project='Empathy', entity='kfq20', name='cg real reward')
step_time = 0
a2c_time = 0
offline_time = 0
test_time = 0
        ...
        with torch.no_grad():
            ...
            for j in range(env.player_num):
                if j == i:
                    ...
                    one_factor = torch.ones((this_eps_len, 1, 1))
                    
                    # real_factor = torch.ones((this_eps_len, 1, 1))
                    total_factor[i].append(origin_factor)
                    weighted_rewards += torch.clamp((one_factor.squeeze() - origin_factor.squeeze()), max=1)*(torch.tensor(reward_batch, dtype=torch.float)) + origin_factor.squeeze() * other_reward[j]
    ...
    if is_wandb:
        wandb.log({'actor loss': total_actor_loss, 
                ...
                #    'hare num':total_hunt_hare_num,
                   ...
                   'episode':ep,
                   'factor_1to2':total_factor[0][0].mean(), #'factor_1to3':total_factor[0][1].mean(), 'factor_1to4':total_factor[0][2].mean(),
                   'factor_2to1':total_factor[1][0].mean(), #'factor_2to3':total_factor[1][1].mean(), 'factor_2to4':total_factor[1][2].mean(),
                #    'factor_3to1':total_factor[2][0].mean(), 'factor_3to2':total_factor[2][1].mean(), 'factor_3to4':total_factor[2][2].mean(),
                #    'factor_4to1':total_factor[3][0].mean(), 'factor_4to2':total_factor[3][1].mean(), 'factor_4to3':total_factor[3][2].mean()
                    })
