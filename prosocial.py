    ...
    for transition_idx in range(env.final_time):
        ...
        for j in range(4):
            if j == id:
                ...
                # factor[j] = torch.tanh(((self_q_value - other_q_value[j]) / torch.mean(abs(self_q_value))).view(-1, 1)).detach()
...
wandb.init(project='Empathy', entity='kfq20', name='0808 test cleanup')
# total_time = 0
# loss_time = 0
# forward_time = 0
