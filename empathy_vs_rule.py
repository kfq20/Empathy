...
env_parallel = [ModifiedCleanupEnv() for _ in range(env_parallel_num)]
env = ModifiedCleanupEnv()
gifting_mode = ['empathy', 'rule_defector', 'rule_cooperator', 'rule_random']
config = {}
...
if is_wandb:
    wandb.init(project='Empathy', entity='kfq20', name='1empathy_1defector_1cooperator_1random')
step_time = 0
a2c_time = 0
offline_time = 0
test_time = 0
...
for ep in range(max_episode):
    ...
    while step < env.final_time:
        ...
        for i in range(env.player_num):
            if 'rule' not in gifting_mode[i]:
                ...
                obs = obs_par[0]
                if gifting_mode[i] == 'rule_cooperator':
                    player_i_pos = np.argwhere(obs[i][i] == 1)[0]
                    player_i_real_pos = env_parallel[0].player_pos[i]
                    if np.sum(obs[i][-3]) == 0:
                ...
                elif gifting_mode[i] == 'rule_defector':
                    player_i_pos = np.argwhere(obs[i][i] == 1)[0]
                    player_i_real_pos = env_parallel[0].player_pos[i]
                    if np.sum(obs[i][-2]) == 0:
                    ...
                    actions.append(np.array([action]))
                elif gifting_mode[i] == 'rule_random':
    ...
    for i in range(env.player_num):
        ...
        elif 'rule' in gifting_mode[i]:
            give_factor = []
            for j in range(env.player_num):
                if j != i:
                    give_factor.append(torch.zeros(this_eps_len*env_parallel_num, 1))
                else:
                    give_factor.append(torch.ones(this_eps_len*env_parallel_num, 1))
