...
target_selfish_agents = [Qnet().to(device) for _ in range(4)]
...
buffer = [ReplayBuffer(capacity=1000000, id=i) for i in range(4)]
...
beta = 1
count = 0
env_obs_mode = 'partial'
window_height = 8
window_width = 5

env = SnowDriftEnv()

wandb.init(project='Empathy', entity='kfq20', name='partial obs')
...
for ep in range(max_episode):
    ...
    while not all_done:
        ...
        for i in range(4):
            ...
            buffer[i].add(obs, actions[i], avail_actions[i], avail_next_actions[i], other_action, reward[i], next_obs, done[i])
        obs = next_obs
        all_done = done[0]
        if buffer[0].size() > minimal_size and done[0]:
            ...
            for i in range(4):
                ...
                other_obs, observable = get_other_obs(obs_i, i)
                ...
                for j in range(4):
                    if j == i:
                        ...
                        l1_score = env.drift_return / torch.sum(abs(other_reward[j]))
                        factor[j] = l1_score * torch.tanh((selfish_q_value - other_q_value[j]) * observable[j].view(-1, 1)).detach()
                ...
                symp_target = (reward + torch.sum(torch.stack([factor[i]*other_reward[i] for i in range(len(factor)) if isinstance(factor[i], torch.Tensor)]), dim=0)) + gamma * symp_q_target * (1-done)
