    ...
    for i in range(4):
        if i == id:
            ...
            for j in range(batch_size):
                if np.sum(obs[j, i]) > 0:
                    observable[j, i] = 1
    other_obs = np.zeros((batch_size, env.player_num, env.channel, env.height, 2*env.window_size+1))
    for i in range(4):
        i_obs = []
        if i == id:
            total_other_obs.append(np.zeros((batch_size, env.channel, env.height, 2*env.window_size+1)))
            ...
            for j in range(batch_size):
                if condition[j]:
                    ...
                    x, y = pos[0][0], pos[1][0]
                    other_obs = obs[j][:, :, max(0, y-env.window_size):min(y+env.window_size+1, 2*env.window_size+1)]
                    if y-env.window_size < 0:
                        other_obs = np.pad(other_obs, ((0, 0), (0, 0), (env.window_size-y, 0)), 'constant', constant_values=((0, 0), (0, 0), (0, 0)))
                    elif y+env.window_size+1 > 15:
                        other_obs = np.pad(other_obs, ((0, 0), (0, 0), (0, y-env.window_size)), 'constant', constant_values=((0, 0), (0, 0), (0, 0)))
                    i_obs.append(other_obs)
                else:
                    i_obs.append(np.zeros((env.channel, env.height, 2*env.window_size+1)))
...
def counterfactual_factor(my_id, other_id, agents, state, last_action, my_action, real_q_value, obsable):
    ...
    other_action = copy.deepcopy(last_action[:, env.action_num*other_id : env.action_num*(other_id+1)])
    virtual_action = torch.nonzero(other_action == 0).view(batch_size, env.action_num-1, 2)
    other_action.fill_(0)
    for i in range(env.action_num-1):
        cur_try_action_pos = virtual_action[:, i, :] # 32*2
        other_action[cur_try_action_pos[:, 0], cur_try_action_pos[:, 1]] = 1 # try another action
        last_action[:, env.action_num*other_id:env.action_num*(other_id+1)] = other_action
...
def get_loss(batch, id, eps):
    ...
    agents_clone = [Qself().to(device) for _ in range(4)]
    agents_target_clone = [Qself().to(device) for _ in range(4)]
    t11 = 0
    t22 = 0
    t33 = 0
    ...
    for transition_idx in range(random_start, random_start + 50):
        ...
        if transition_idx == 0:
            last_action = np.zeros((episode_num, env.player_num, env.action_num))
        else:
            last_action = action[:, transition_idx - 1]
            last_action = np.eye(env.action_num)[last_action] # one-hot
        ...
        this_action = np.eye(env.action_num)[this_action]
        ...
        for j in range(4):
            if j == id:
                ...
                # j_imagine_obs[j] = agents_imagine[id](other_obs[j]).view(batch_size, env.channel, env.height, 2*env.window_size+1)
                # j_next_imagine_obs[j] = agents_imagine[id](next_other_obs[j]).view(batch_size, env.channel, env.height, 2*env.window_size+1)
                ...
                j_action_onehot = F.one_hot(j_action[j], num_classes=env.action_num).squeeze()
    ...
    return self_loss, symp_loss

wandb.init(project='Empathy', entity='kfq20', name='modify cleanup')
total_step_time = 0
...
for ep in range(max_episode):
    ...
    total_punish_num = 0
    last_action = np.zeros((env.player_num, env.action_num))
    step = 0
    ...
    while not all_done:
        ...
        env_s = time.time()
        next_obs, reward, done, collect_waste_num = env.step(actions)
        env_e = time.time()
        ...
        one_hot = np.zeros((len(actions), env.action_num))
    ...
    for i in range(step, env.final_time):
        for j in range(4):
            h[j].append(np.zeros((1, 128)))
            o[j].append(np.zeros((env.player_num, env.channel, env.height, 2*env.window_size+1)))
            a[j].append([0,0,0,0])
            r[j].append(0)
            o_next[j].append(np.zeros((env.player_num, env.channel, env.height, 2*env.window_size+1)))
            avail_a[j].append(np.zeros(env.action_num))
            avail_a_next[j].append(np.zeros(env.action_num))
            other_a[j].append(np.zeros(4))
            d[j].append(0)
            pad[j].append(1)

    for i in range(4):
        buffer[i].add(o[i], a[i], avail_a[i], avail_a_next[i], other_a[i], r[i], o_next[i], d[i], pad[i], h[i])
    ...
    if buffer[0].size() > minimal_size:
        ...
        wandb.log({'reward_1':total_reward[0], 'reward_2':total_reward[1], 'reward_3':total_reward[2], 'reward_4':total_reward[3], 'total_reward':sum(total_reward), 'collect_waste':total_collect_waste_num})
        for i in range(4):
            obs, action, avail_action, avail_next_action, other_action, reward, next_obs, done, pad, hidden_state = buffer[i].sample(min(buffer[i].size(), batch_size))
            episode_num = obs.shape[0]
            ...
            self_loss, symp_loss = get_loss(batch, i, ep)
        ...
        if count % target_update == 0:
            for i in range(4):
                target_selfish_agents[i].load_state_dict(selfish_agents[i].state_dict())
                target_symp_agents[i].load_state_dict(symp_agents[i].state_dict())
        wandb.log({'self_loss': total_self_loss, 'symp_loss': total_symp_loss})
