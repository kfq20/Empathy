...
env_parallel = [StagHuntEnv() for _ in range(env_parallel_num)]
env = StagHuntEnv()
...
def other_obs_process(obs, id):
    ...
    if env.name == 'mp_cleanup':
        ...
        i_alive = np.any(obs[:, id, id, :, :] == 1, axis=(1, 2))
        for j in range(env.player_num):
            if j == id:
                continue
            else:
                j_observable = np.any(obs[:, id, j, :, :] == 1, axis=(1, 2))
                observable[:, j] = j_observable*i_alive
        for i in range(env.player_num):
            if i == id:
...
def counterfactual_factor(my_id, other_id, selfish_agents, imagine_nn, state, this_action, real_q_value, observable):
    ...
    observable = observable.view(factor.shape[0], factor.shape[1])
    # factor = 2 * (real_q_value - min_value) / (max_value - min_value) - 1
    return (factor*observable).detach().cpu()
...
if is_wandb:
    wandb.init(project='Empathy', entity='kfq20', name='final_empathy_staghunt_seed_3')
step_time = 0
a2c_time = 0
offline_time = 0
test_time = 0
...
for ep in range(max_episode):
    ...
    player_step_len = [1 for _ in range(env.player_num)]
    for i in range(env.player_num):
    ...
    this_eps_len = step
    print("eps:", ep, "len:", player_step_len)
    ...
    for i in range(env.player_num):
        ...
        with torch.no_grad():
            ...
            _, observable = other_obs_process(obs_batch.reshape(obs_batch.shape[0]*obs_batch.shape[1], obs_batch.shape[2], obs_batch.shape[3], obs_batch.shape[4], obs_batch.shape[5]), i)
            observable = torch.tensor(observable, dtype=torch.float).to(device)
                ...
                for j in range(env.player_num):
                    if j == i:
                        ...
                        if gifting_mode[i] == 'empathy':
                            origin_factor = counterfactual_factor(i, j, selfish_agents, agents_imagine[i], state, this_action, self_q_value, observable[j]) # len(eps)-1
            ...
            back_e = time.time()
            # backward_time += back_e - back_s
    for i in range(env.player_num):
        if player_step_len[i] > 1:
            i_log_probs = torch.cat(log_probs[i][:player_step_len[i]], dim=0)
        else:
            i_log_probs = torch.cat(log_probs[i][:player_step_len[i]], dim=0)
        i_values = torch.cat(values[i][:player_step_len[i]], dim=0).squeeze()
        
        weighted_rewards = 0
        for j in range(env.player_num):
            j_reward = torch.tensor(r[j][:player_step_len[i]], dtype=torch.float).view(player_step_len[i]*env_parallel_num, -1)
            weighted_rewards += total_give_factor[j][i][:player_step_len[i]].squeeze() * j_reward.squeeze()
        # compute returns
        state = torch.tensor(obs_par[:, i], dtype=torch.float).to(device)
        h_in = symp_agents[i].hx.to(device)
        c_in = symp_agents[i].cx.to(device)
        ...
        returns = []
        weighted_rewards = weighted_rewards.view(player_step_len[i], env_parallel_num).to(device)
        d[i] = torch.tensor(d[i], dtype=torch.float).to(device)
        for step in reversed(range(player_step_len[i])):
            # a = 1-d[i][step, :]
            # b = R
