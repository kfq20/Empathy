...
# wandb.init(project='Empathy', entity='kfq20', name='sd_test')
            ...
            for i in range(4):
                ...
                for j in range(4):
                    if j == i:
                        ...
                        other_q_value[j] = j_q[j].gather(1, (j_action[j] * observable[j]).to(torch.int64).view(-1, 1)) * observable[j].view(-1, 1)
                ...
                loss = torch.mean(F.mse_loss(total_q_value, target))
