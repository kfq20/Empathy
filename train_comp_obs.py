...
wandb.init(project='Empathy', entity='kfq20', name='comp obs')
    ...
    while not all_done:
        ...
        if buffer[0].size() > minimal_size and done[0]:
            ...
            for i in range(4):
                ...
                q_value_selfish = selfish_agents[i](obs_i).gather(1, action)

                # max_next_q_value = agents_target[i](next_obs).max(1)[0].view(-1, 1)
                q_target_selfish = selfish_agents_target[i](next_obs_i)
                ...
                for j in range(4):
                    if j == i:
                        ...
                        agents_clone[j].load_state_dict(selfish_agents_target[i].state_dict())
                        ...
                        factor[j] = torch.tanh((q_value_selfish - other_q_value[j]).view(-1, 1)).detach()
                ...
                target = reward + gamma * (total_q_target) * (1 - done)
                loss = torch.mean(F.mse_loss(total_q_value, target.detach()))
            ...
            wandb.log({'dqn_loss': total_loss, 'imagine_loss':imagine_loss})