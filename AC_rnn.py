...
selfish_agents = [QselfRNN(config).to(device) for _ in range(4)]
target_selfish_agents = [QselfRNN(config).to(device) for _ in range(4)]
for i in range(4):
...
def get_loss(batch, id, eps):
    ...
    for transition_idx in range(env.final_time):
        ...
        self_q_value, selfish_agents[id].hidden = selfish_agents[id](state, last_action, selfish_agents[id].hidden)
        self_q_value = self_q_value.gather(1, action_i)
        self_q_target, target_selfish_agents[id].hidden = target_selfish_agents[id](next_state, this_action, target_selfish_agents[id].hidden)
        ...
        agents_clone = [QselfRNN(config).to(device) for _ in range(4)]
        ...
        for j in range(4):
            if j == id:
                ...
                agents_clone[j].hidden = selfish_agents[i].hidden
                # j_imagine_obs[j] = j_imagine_obs[j].view(episode_num, -1)
                # j_inputs = torch.cat((j_imagine_obs[j], this_action), dim=1)
                j_q[j], agents_clone[j].hidden = agents_clone[j](j_imagine_obs[j], last_action, agents_clone[j].hidden)
...
# wandb.init(project='Empathy', entity='kfq20', name='A2C test v2', notes="self q rnn")
step_time = 0
a2c_time = 0
offline_time = 0
test_time = 0
...
for ep in range(max_episode):
    ...
    for i in range(4):
        ...
        j_next_imagine_obs = [0 for _ in range(4)]
        agents_clone = [QselfRNN(config).to(device) for _ in range(4)]
        agents_target_clone = [QselfRNN(config).to(device) for _ in range(4)]
        j_action = [0 for _ in range(4)]
        j_q = [0 for _ in range(4)]
        ...
        factor = [0 for _ in range(4)]
        weighted_rewards = []
        selfish_agents[i].init_hidden(1)
        target_selfish_agents[i].init_hidden(1)
        ...
        with torch.no_grad():
            for t in range(len(rewards)):
                ...
                self_q_value, selfish_agents[i].hidden = selfish_agents[i](state, last_action, selfish_agents[i].hidden)
                self_q_value = self_q_value.gather(1, action_i)
                ...
                for j in range(4):
                    if j == i:
                        ...
                        obs_j = torch.tensor(o[j][t][j], dtype=torch.float).to(device)
                        next_obs_j = torch.tensor(o_next[j][t][j], dtype=torch.float).to(device)
                        j_imagine_obs[j] = agents_imagine[i](obs_j).view(1, env.channel, window_height, window_width)
                        j_next_imagine_obs[j] = agents_imagine[i](next_obs_j).view(1, env.channel, window_height, window_width)
                        j_action[j] = other_action_transition[j]
                        j_action_onehot = F.one_hot(j_action[j], num_classes=env.action_space).squeeze()
                        agents_clone[j].load_state_dict(selfish_agents[i].state_dict())
                        agents_clone[i].hidden = selfish_agents[i].hidden
                        # agents_clone[j].hidden = selfish_agents[i].hidden
                        # j_imagine_obs[j] = j_imagine_obs[j].view(1, -1)
                        # j_inputs = torch.cat((j_imagine_obs[j], last_action), dim=1)
                        j_q[j], _= agents_clone[j](j_imagine_obs[j], last_action, agents_clone[i].hidden)
                        j_pi = F.softmax(j_q[j], dim=1)
                        ...
                        agents_target_clone[j].hidden = target_selfish_agents[i].hidden
                        # agents_target_clone[j].hidden = target_selfish_agents[i].hidden
                        # j_next_imagine_obs[j] = j_next_imagine_obs[j].view(1, -1)
                        # next_j_inputs = torch.cat((j_next_imagine_obs[j], this_action), dim=1)
                        next_j_q_value, _ = agents_target_clone[j](j_next_imagine_obs[j], this_action, agents_target_clone[j].hidden)
                        other_q_target[j] = next_j_q_value.max(1)[0].view(-1, 1).detach()
                        other_reward[j] = (other_q_value[j] - gamma * other_q_target[j]).detach()
                        other_reward_log[j] = other_reward[j][0].item()
        ...
        for t in reversed(range(len(rewards))):
            R = weighted_rewards[t].detach() + 0.99 * R
            returns[t] = R
    ...
    offline_e = time.time()
    offline_time += offline_e - offline_s

    # print("eps:", ep, "===================")
    # print("step time", step_time)
    # print("a2c time", a2c_time)
