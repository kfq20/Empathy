...
def counterfactual_factor(my_id, other_id, agents, state, last_action, my_action, real_q_value, obsable):
    ...
    virtual_action = torch.nonzero(other_action == 0).view(32, 5, 2)
    ...
    for transition_idx in range(env.final_time):
        ...
        for j in range(4):
            if j == id:
                ...
                obs_j = other_obs[j]
                next_obs_j = next_other_obs[j]
                # j_imagine_obs[j] = agents_imagine[id](other_obs[j]).view(batch_size, env.channel, window_height, window_width)
                # j_next_imagine_obs[j] = agents_imagine[id](next_other_obs[j]).view(batch_size, env.channel, window_height, window_width)
                ...
                j_inputs = torch.cat((obs_j.view(episode_num, -1), last_action), dim=1)
                ...
                next_j_inputs = torch.cat((next_obs_j.view(episode_num, -1), this_action), dim=1)
...
wandb.init(project='Empathy', entity='kfq20', name='partial no img')
# total_time = 0
# loss_time = 0
# forward_time = 0
