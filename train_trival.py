...
wandb.init(project='Empathy', entity='kfq20', name='cooperate cleanup')
...
for ep in range(max_episode):
    ...
    while not all_done:
        ...
        for i in range(4):
            ...
            buffer[i].add(obs, actions[i], avail_actions[i], avail_next_actions[i], other_action, reward, next_obs, done[i])
        obs = next_obs
        all_done = done[0]
        if buffer[0].size() > minimal_size and done[0]:
            ...
            for i in range(4):
                ...
                reward_i = torch.tensor(reward[:, i], dtype=torch.float).to(device)
                ...
                reward = torch.tensor(reward, dtype=torch.float).permute(1,0).to(device)
                ...
                # # print(other_reward)
                x = (reward[0]+reward[1]+reward[2]+reward[3]).view(-1, 1)
                selfish_target = x + gamma * selfish_q_target * (1 - done)
            ...
            wandb.log({'self_loss': total_self_loss})
