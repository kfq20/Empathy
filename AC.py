...
wandb.init(project='Empathy', entity='kfq20', name='A2C', notes="normalize reward")
step_time = 0
a2c_time = 0
offline_time = 0
test_time = 0
            ...
            for t in reversed(range(len(rewards))):
                ...
                for j in range(4):
                    if j == i:
                        ...
                        j_q[j] = agents_clone[j](j_imagine_obs[j], last_action)
                        max_q_value = torch.max(j_q[j]).item()
                        min_q_value = torch.min(j_q[j]).item()
                        # j_pi = F.softmax(j_q[j], dim=1)
                        ...
                        next_j_q_value = agents_target_clone[j](j_next_imagine_obs[j], this_action)
                        other_q_target[j] = next_j_q_value.max(1)[0].view(-1, 1).detach()
                        other_reward[j] = (j_q[j] - gamma * other_q_target[j]).detach()
                        max_reward = torch.max(other_reward[j])
                        min_reward = torch.min(other_reward[j])
                        real_reward = other_reward[j].gather(1, (j_action[j]).view(-1, 1)).detach()

                        # scaled_value = (element - min_value) * (new_max - new_min) / (max_value - min_value) + new_min
                        other_reward[j] = (real_reward - min_reward) * (MAX_REWARD - MIN_REWARD) / (max_reward - min_reward) - 1
