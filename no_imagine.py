...
def counterfactual_factor(my_id, other_id, agents, state, last_action, my_action, real_q_value):
    counterfactual_result = []
    counterfactual_result.append(real_q_value)
    other_action = copy.deepcopy(last_action[:, 6*other_id:6*(other_id+1)])
    virtual_action = torch.nonzero(other_action == 0).view(batch_size, 5, 2)
    ...
    for i in range(5):
        ...
        last_action[:, 6*other_id:6*(other_id+1)] = other_action
    ...
    return factor.view(-1, 1).to(device).detach()
...
def get_loss(batch, id, eps):
    ...
    for transition_idx in range(env.final_time):
        ...
        if transition_idx == 0:
            last_action = np.zeros((episode_num, 4, 6))
        else:
            last_action = action[:, transition_idx - 1]
            last_action = np.eye(6)[last_action] # one-hot
        ...
        this_action = np.eye(6)[this_action]
        ...
        for j in range(4):
            if j == id:
                ...
                obs_j = torch.tensor(obs_transition[:, j, :, :, :], dtype=torch.float).to(device).view(episode_num, -1)
                next_obs_j = torch.tensor(next_obs_transition[:, j, :, :, :], dtype=torch.float).to(device).view(episode_num, -1)
                ...
                # j_action_onehot = F.one_hot(j_action[j], num_classes=6).squeeze()
                ...
                if transition_idx == 0:
                    factor[j] = torch.zeros((batch_size, 1)).to(device).detach()
                ...
                # factor[j] = cleanup_compute_factor(obs_i, obs_j, next_obs_i, next_obs_j, action_i, j_action[j], id, j)
                    ...
                    factor[j] = counterfactual_factor(id, j, selfish_agents, state, last_action, action_i, self_q_value)
...
wandb.init(project='Empathy', entity='kfq20', name='total obs no img')
# total_time = 0
# loss_time = 0
# forward_time = 0
for ep in range(max_episode):
    ...
    for i in range(step, env.final_time):
        for j in range(4):
            ...
            avail_a[j].append(np.zeros(6))
            avail_a_next[j].append(np.zeros(6))
